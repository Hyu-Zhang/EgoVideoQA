...
from .patch_selection import CustomModule
...
class VarAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 initialize='random', dim_text=None, norm_layer=nn.LayerNorm, space_attn=True, patch_mask=None):
        super().__init__()
...
#             self.norm_i2t_out = norm_layer(dim)
        
        self.cls_mask = torch.tensor([1.]).cuda()
        
    def forward(self, x, einops_from, einops_to, einops_dims, y=None, y_mask=None, patch_mask=None):
...
#         print(q.shape, k.shape, v.shape) # torch.Size([32, 3137, 768])
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (q, k, v))
#         print(q.shape, k.shape, v.shape) # torch.Size([384, 3137, 64])
        q = q*self.scale

        # splice out CLS token at index 1
        (cls_q, q_), (cls_k, k_), (cls_v, v_) = map(lambda t: (t[:, 0:1], t[:, 1:]), (q, k, v))
#         print(cls_q.shape) # torch.Size([384, 1, 64])
        
        cls_patch_mask = None
        # add
        if patch_mask != None:
            temp_patch_mask = patch_mask.clone()
            temp_patch_mask[patch_mask == 0] = -torch.exp(torch.tensor(12.0)) # torch.Size([32, 3137])
            temp_patch_mask = temp_patch_mask.unsqueeze(1).repeat(1, h, 1).view(-1, q.size(1)) # torch.Size([384, 3137])
            cls_patch_mask = temp_patch_mask.unsqueeze(1)
            
        # let CLS token attend to key / values of all patches across time and space
        cls_out, score = attn(cls_q, k, v, cls_patch_mask)
        # rearrange across time or space
#         print(q_.shape, k_.shape, v_.shape) # torch.Size([384, 3136, 64])
        q_, k_, v_ = map(lambda t: rearrange(t, f'{einops_from} -> {einops_to}', **einops_dims), (q_, k_, v_))
#         print(q_.shape, k_.shape, v_.shape) # torch.Size([75264, 16, 64])

        # add
        if patch_mask != None:
            new_patch_mask = rearrange(temp_patch_mask[:,1:], f'{einops_from} -> {einops_to}'.replace(' d', ''), **einops_dims)
            new_patch_mask = torch.cat((self.cls_mask.unsqueeze(0).repeat(q_.size(0), 1), new_patch_mask), dim=-1)
            cls_patch_mask = new_patch_mask.unsqueeze(1).repeat(1, q_.size(1), 1)
...
#         print(q_.shape, k_.shape, v_.shape) # torch.Size([75264, 17, 64])
        
        # attention
        out, _ = attn(q_, k_, v_, cls_patch_mask)
...
#         print("-----------",dim_text)
    ...
    def forward(self, x, einops_from_space, einops_to_space, einops_from_time, einops_to_time, time_n, space_f, patch_mask=None, y=None, y_mask=None):

        einops_dims = {'f': space_f, 'n':time_n}
        time_output, _ = self.timeattn(self.norm3(x), einops_from_time, einops_to_time, einops_dims=einops_dims, y=None, y_mask=None, patch_mask=None)
        time_residual = x + time_output
        space_output, score = self.attn(self.norm1(time_residual), einops_from_space,
                                 einops_to_space, einops_dims=einops_dims, y=y, y_mask=y_mask, patch_mask=patch_mask)

        score = score[:, :, :-1].view(x.size(0), self.attn.num_heads, 1, space_f, time_n)
        score = score.mean(dim=2).mean(dim=1)
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.large_pos_embed = nn.Parameter(torch.zeros(1, 49, embed_dim))
        self.large_pos_embed.data.copy_(self.pos_embed.data[:, 1:50, :])

#         self.large_pos_embed = nn.Parameter(torch.zeros(1, 25, embed_dim))
        ...
        self.cloned_blocks = nn.ModuleList([copy.deepcopy(block) for block in self.blocks[:6]])
        ...
        trunc_normal_(self.large_pos_embed, std=.02)
        ...
        # add
        self.patch_selection = CustomModule(embed_dim, 2, 0.15, 196)
