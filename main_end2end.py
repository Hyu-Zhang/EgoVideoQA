...
from utils.loss import SmoothingCrossEntropyLoss
...
writer = SummaryWriter('/userhome/runs/')
...
def parse_args():
    ...
    parser.add_argument("--basedir", type=str, default='/userhome/output/',
...
def main_worker(gpu, args):
    ...
    dataset_train = EgoTaskQA(dataset_name="EgoTaskQA", text_params={"input": "text"},
                                  video_params={"input_res": args.frame_resolution,
                                                ...
                                                "loading": "lax"},
                                  data_dir="/userhome/data/qa_videos",
                                  meta_dir="/userhome/data/Data/qa/" + args.dataset_split_type,
                                  tsfms=init_video_transform_dict()['test'], reader='decord', split='train', neg_param=60, args=args)
    
    sampler_train = torch.utils.data.distributed.DistributedSampler(dataset_train)
    dataloader_train = DataLoader(dataset_train, batch_size=args.per_gpu_batch_size, drop_last=True, sampler=sampler_train, pin_memory=True, collate_fn=collate_func, num_workers=8)
    ...
    dataloader_val = DataLoader(dataset_val, batch_size=args.per_gpu_batch_size, drop_last=False, sampler=sampler_val, collate_fn=collate_func, num_workers=8)
    ...
    dataloader_test = DataLoader(dataset_test, batch_size=args.per_gpu_batch_size, drop_last=False, sampler=sampler_test, collate_fn=collate_func, num_workers=8)
    ...
    with open('/userhome/data/Data/qa/' + args.dataset_split_type + '/answer_set.txt', 'r') as ansf:
    ...
    with open('/userhome/data/Data/qa/' + args.dataset_split_type + '/all_reasoning_types.txt', 'r') as reasonf:
    ...
    unique_dict = torch.load('/userhome/pretrain_model/reasoning_unique_cat.pth')
        ...
        for idx, (video_frames, text_tokens, attention_masks, answers, reasoning_type_lst) in tqdm(enumerate(dataloader_train)):
            ...
            with torch.set_grad_enabled(True):
                with torch.cuda.amp.autocast():
                    ...
                    logits = args.allgather(logits, gpu, args)  # torch.Size([64, 899])
                    answers = args.allgather(answers, gpu, args) #  torch.Size([64])
...
#             nn.utils.clip_grad_value_(model.parameters(), 1.0)
            ...
            writer.add_scalar('train_acc', train_acc.item(), global_step = global_step)
            ...
            if (global_step+1) % args.i_val == 0:
                ...
                writer.add_scalar('val_acc', val_acc.item(), global_step = global_step)
                writer.add_scalar('val_loss', val_loss.item(), global_step = global_step)
...
def main():
    ...
    if 'SLURM_JOB_ID' in os.environ:
        ...
        args.rank = 0
        args.dist_url = 'tcp://localhost:2355'
